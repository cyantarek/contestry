...
func GenerateToken(username, userType string, userId, validity int, signkey *rsa.PrivateKey) (string, error) {
	claims := &jwt.MapClaims{"exp": time.Now().Add(time.Minute*1), "user_id": userId, "username": username, "user_type": userType}
